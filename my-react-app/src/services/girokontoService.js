...
export async function ladeGirokonto() {
...
export async function girokontoHinzufuegen(formData) {
    try {
        const { data: { user } } = await supabase.auth.getUser();
        ...
        if (handleApiError(giroError, "Girokonto anlegen")) return false;
...
export async function girokontoSpeichern(assetId, formData) {
    ...
    if (handleApiError(giroError, "Girokontodaten updaten")) return false;
